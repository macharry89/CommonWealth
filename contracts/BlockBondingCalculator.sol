...
contract BlockBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(Block) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(Block), "not a Block lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** Block.decimals() ) ).div( getTotalValue( _pair ) );
    }
}
